...
EBTNodeResult::Type UBTTask_GetOwnable::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory) {
	...
	if (bestChoice != ontF.GetHands() && bestChoice != entity->GetGrabbedItem()) {
		...
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Cyan, TEXT("Something Worthy!\n   ") + bestChoice->GetOwner()->GetActorLabel());
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Cyan, TEXT("   score: ") + FString::SanitizeFloat(bestChoiceAffordance));
	}
